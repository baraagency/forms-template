...
- Appointment Type options use FUB `/appointmentTypes` element `id` as the select value.
...
- SISU writes require enabled mappings with non-empty `sisu_field_name`; normalize Buyer/Seller to `client_type` `0`/`1` and `type_id` `b`/`s`.
...
- Demo mode (UI label, not "Local demo mode") is active only when `DEMO_MODE=true`.
