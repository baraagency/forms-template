import { updateDealMapping } from "@/app/api/_services/settingsQueries";
...
export async function action({
  ...
  const result = await updateDealMapping(id, patch);
  if (result.error || !result.data) {
    const status = result.error === "Mapping not found." ? 404 : 500;
    return Response.json(
      { message: result.error ?? "Failed to update deal mapping." },
      { status },
    );
  }
